                ...
                case TDATA_POINT:
                    {
                        if(pPropertyInfo->GetPublic())
                        {
                            NFMsg::PropertyPoint* pDataObject = pPublicData->add_property_point_list();
                            pDataObject->set_property_name(pPropertyInfo->GetKey());
                            *(pDataObject->mutable_data()) = NFINetModule::NFToPB(pPropertyInfo->GetPoint());
                        }

                        if(pPropertyInfo->GetPrivate())
                        {
                            NFMsg::PropertyPoint* pDataObject = pPrivateData->add_property_point_list();
                            pDataObject->set_property_name(pPropertyInfo->GetKey());
                            *(pDataObject->mutable_data()) = NFINetModule::NFToPB(pPropertyInfo->GetPoint());
                        }
                    }
    ...
    case TDATA_POINT:
        {
            ...
            *pDataObject->mutable_data() = NFINetModule::NFToPB(newVar.GetObject());
            ...
            for(int i = 0; i < valueBroadCaseList.GetCount(); i++)
            {
                ...
                SendMsgPBToGate(NFMsg::EGMI_ACK_PROPERTY_POINT, xPropertyObject, identOld);
            }
        }
        break;
            ...
            if(xRecord)
            {
                ...
                if(xRecord->QueryRow(nRow, xRowDataList))
                {
                    for(int i = 0; i < xRowDataList.GetCount(); i++)
                    {
                        ...
                        case TDATA_POINT:
                            {
                                const Point3D& value = xRowDataList.Point(i);
                                //if (!identValue.IsNull())
                                {
                                    ...
                                    *pAddData->mutable_data() = NFINetModule::NFToPB(value);
                                }
                            }
                            break;
                        default:
            ...
            case TDATA_DOUBLE:
                {
                    ...
                    *xRecordChanged.mutable_player_id() = NFINetModule::NFToPB(self);
                    ...
                    xRecordChanged.set_record_name(strRecordName);
                    NFMsg::RecordFloat* recordProperty = xRecordChanged.add_record_list();
                    recordProperty->set_row(nRow);
                    recordProperty->set_col(nCol);
            ...
            case TDATA_POINT:
                {
                    ...
                    *recordProperty->mutable_data() = NFINetModule::NFToPB(newVar.GetPoint());
                    ...
                    for(int i = 0; i < valueBroadCaseList.GetCount(); i++)
                    {
                        ...
                        SendMsgPBToGate(NFMsg::EGMI_ACK_RECORD_POINT, xRecordChanged, identOther);
                    }
                }
                break;
            default:
    ...
    if(nOldGroupID > 0)
    {
        ...
        if(valueAllOldObjectList.GetCount() > 0)
        {
            ...
            for(int i = 0; i < valueAllOldObjectList.GetCount(); i++)
            {
                ...
                {
                    valueAllOldObjectList.Set(i, NULL_GUID);
                }
...
int NFCGameServerNet_ServerModule::OnContainerEvent(const NFGUID& self, const std::string& strPropertyName, const NFIDataList::TData& oldVar, const NFIDataList::TData& newVar)
{
    ...
    for(int i = 0; i < valueOldAllObjectList.GetCount(); i++)
    {
        ...
        if(identBC == self)
        {
            valueOldAllObjectList.Set(i, NULL_GUID);
        }
    }
...
int NFCGameServerNet_ServerModule::OnSwapSceneResultEvent(const NFGUID& self, const int nEventID, const NFIDataList& var)
{
    ...
    const Point3D& xPoint = var.Point(4);
    ...
    xSwapScene.set_x(xPoint.x);
    xSwapScene.set_y(xPoint.y);
    xSwapScene.set_z(xPoint.z);
