...
void NFCWorldNet_ServerModule::OnGameServerUnRegisteredProcess(const int nSockIndex, const int nMsgID, const char* msg, const uint32_t nLen)
{
    NFGUID nPlayerID;
    NFMsg::ServerInfoReportList xMsg;
    if(!m_pNetModule->ReceivePB(nSockIndex, nMsgID, msg, nLen, xMsg, nPlayerID))
...
void NFCWorldNet_ServerModule::OnRefreshGameServerInfoProcess(const int nSockIndex, const int nMsgID, const char* msg, const uint32_t nLen)
{
    NFGUID nPlayerID;
    NFMsg::ServerInfoReportList xMsg;
    if(!m_pNetModule->ReceivePB(nSockIndex, nMsgID, msg, nLen, xMsg, nPlayerID))
...
void NFCWorldNet_ServerModule::OnProxyServerRegisteredProcess(const int nSockIndex, const int nMsgID, const char* msg, const uint32_t nLen)
{
    NFGUID nPlayerID;
    NFMsg::ServerInfoReportList xMsg;
    if(!m_pNetModule->ReceivePB(nSockIndex, nMsgID, msg, nLen, xMsg, nPlayerID))
...
void NFCWorldNet_ServerModule::OnProxyServerUnRegisteredProcess(const int nSockIndex, const int nMsgID, const char* msg, const uint32_t nLen)
{
    NFGUID nPlayerID;
    NFMsg::ServerInfoReportList xMsg;
    if(!m_pNetModule->ReceivePB(nSockIndex, nMsgID, msg, nLen, xMsg, nPlayerID))
...
void NFCWorldNet_ServerModule::OnRefreshProxyServerInfoProcess(const int nSockIndex, const int nMsgID, const char* msg, const uint32_t nLen)
{
    NFGUID nPlayerID;
    NFMsg::ServerInfoReportList xMsg;
    if(!m_pNetModule->ReceivePB(nSockIndex, nMsgID, msg, nLen, xMsg, nPlayerID))
...
void NFCWorldNet_ServerModule::LogGameServer()
{
    ...
    m_pLogModule->LogInfo(NULL_GUID, "Begin Log GameServer Info", "");
    ...
    while(pGameData)
    {
        ...
        m_pLogModule->LogInfo(NULL_GUID, stream);

        pGameData = mGameMap.Next();
    }

    m_pLogModule->LogInfo(NULL_GUID, "End Log GameServer Info", "");

    m_pLogModule->LogInfo(NULL_GUID, "Begin Log ProxyServer Info", "");
    ...
    m_pLogModule->LogInfo(NULL_GUID, "End Log ProxyServer Info", "");
}
...
int NFCWorldNet_ServerModule::OnObjectListEnter(const NFIDataList& self, const NFIDataList& argVar)
{
    ...
    for(int i = 0; i < argVar.GetCount(); i++)
    {
        ...
        *(pEntryInfo->mutable_object_guid()) = NFINetModule::NFToPB(identOld);
        ...
        pEntryInfo->set_x(m_pKernelModule->GetPropertyDouble(identOld, "X"));
        pEntryInfo->set_y(m_pKernelModule->GetPropertyDouble(identOld, "Y"));
        pEntryInfo->set_z(m_pKernelModule->GetPropertyDouble(identOld, "Z"));
        ...
        if(pRecord->IsUsed(i))
        {
            ...
            for(int j = 0; j < pRecord->GetCols(); j++)
            {
                ...
                case TDATA_TYPE::TDATA_POINT:
                    {
                        const Point3D& ident = pRecord->GetPoint(i, j);
                        //if ( !ident.IsNull() )
                        {
